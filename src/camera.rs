...
#[require(Camera3d, PanOrbitState, PanOrbitSettings)]
...
pub struct PanOrbitSettings {
    ...
    pub zoom_key: Option<MouseButton>,
...
impl Default for PanOrbitSettings {
    fn default() -> Self {
        PanOrbitSettings {
            ...
            orbit_sensitivity: 0.3f32.to_radians(), // 0.1 degree per pixel
...
pub fn pan_orbit_camera(
    ...
    for (settings, mut state, mut transform) in &mut q_camera {
        ...
        if settings.pan_key.map(|key| mouse.pressed(key)).unwrap_or(false) {
        ...
        if settings.orbit_key.map(|key| mouse.pressed(key)).unwrap_or(false) {
        ...
        if settings.zoom_key.map(|key| mouse.pressed(key)).unwrap_or(false) {
...
pub fn focus_selected_entity(
    input: Res<ButtonInput<KeyCode>>,
    entity_selected: Res<EntitySelected>,
    global_query: Query<&GlobalTransform>,
    mut cam_query: Query<(&mut PanOrbitState, &mut Transform)>,
) {
    ...
    // cam.radius = 10.0;
    cam_transform.translation = cam.center + cam_transform.rotation.mul_vec3(Vec3::new(0.0, 0.0, cam.radius));
}